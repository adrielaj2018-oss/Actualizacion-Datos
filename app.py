...
def captura():
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-7"><div class="card card-pro p-4"><h5 class="fw-bold"><i class="bi bi-person-lines-fill me-2"></i>Datos encontrados</h5><div id="msg" class="alert alert-info">Esperando captura de DNI.</div>
        <form id="frm" style="display:none" onsubmit="guardarDatos(event)"><input type="hidden" id="metodo_captura" value="DIGITACION"><div class="row g-3">
          ...
          <div class="col-md-7"><label class="fw-bold">Correo electrónico</label><div class="input-group"><input class="form-control" id="correo_usuario" placeholder="USUARIO" autocomplete="off"><select class="form-select" id="correo_dominio" style="max-width:180px"><option value="@gmail.com">@gmail.com</option><option value="@hotmail.com">@hotmail.com</option><option value="@outlook.com">@outlook.com</option></select></div><input type="hidden" id="correo"></div><div class="col-md-5"><label class="fw-bold">Celular</label><input class="form-control" id="celular" placeholder="999999999" maxlength="20"></div>
    ...
    function armarCorreo(){ const u=(correo_usuario.value||'').replace(/@.*/, '').replace(/\s/g,'').toLowerCase(); correo_usuario.value=u; correo.value = u ? (u + correo_dominio.value) : ''; return correo.value; }
...
def api_guardar():
    ...
    if correo and not re.match(r'^[^@\s]+@(?:gmail\.com|hotmail\.com|outlook\.com)$', correo): return jsonify(ok=False,msg='Correo inválido. Use @gmail.com, @hotmail.com o @outlook.com.')
